...
- "title" MUST be a short human-readable name (~2–8 words, max ~60 chars). NEVER return the URL itself as the title. Derive the title in this priority order: (1) the doc name if the message text mentions it (e.g. "the design doc template" → "Design Doc Template"); (2) the last meaningful segment of the URL path, humanized (e.g. github.com/kubernetes/enhancements → "kubernetes/enhancements"; …/README.md → "README.md"); (3) as a last resort, the type label + host (e.g. "Google Doc — docs.google.com"). If the raw URL is the only sensible label, still shorten it — do not emit a full URL as a title.
